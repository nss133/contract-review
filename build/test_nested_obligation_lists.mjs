...
try{
 ...
 const report=SafetyRuntime.standardReport().rows.find(r=>r.check_id==='ITSEC-12');check(report.evidence.some(e=>e.text===intro),'intro not in proof');check(report.evidence.some(e=>e.text==='1. 다음 각 목의 사항을 이행할 것'),'nested intro not in proof');
 for(const bad of [list.replace('나.','다.'),list.replace('나.','가.'),list.replace('경우','경우에만'),list.replace('수탁자는','위탁자는'),list.replace('이행하여야 한다','이행할 수 있다')]){state.subDocs[0].text=bad;runAnalysis();check(!verdictStore['ITSEC-12']?.verdict,'bad list still completed');}
 ...
 try{const dt=new DataTransfer();dt.items.add(new File(['synthetic'],'중첩목록표준.docx'));e('template-file').files=dt.files;await e('template-file').onchange();}finally{extractFileStructure=extract;}
 ...
 await analyze(secret.replace('서울','부산').replace('제2항','제1항'));check(!verdictStore['CNS-SECRET']?.verdict,'changed item reference not revoked');
 await analyze(secret.replace('서울','부산').replace('가목','나목'));check(!verdictStore['CNS-SECRET']?.verdict,'missing item reference not revoked');
 const saved=TemplateLibraryRuntime.get(),prior=saved.templates.find(t=>t.name==='중첩목록표준.docx');check(!!prior,'migration source');prior.registration.version=12;prior.bindings=[];localStorage.setItem('cr-template-library-v1',JSON.stringify(saved));
 return {packaged_questions:cases.length,actual_questions:actual,list_annex_completed:true,source_intro_preserved:true,number_condition_role_modality_revocation:true,registered_without_approval:true,standard_to_sentence:true,human_preserved:true,paragraph_item_letter_reference_precedent:true,changed_reference_revocation:true,second_column:true,pending_resolved:true};
 })()`);
 await run('location.reload()');await new Promise(r=>setTimeout(r,300));await ready();
 const migration=await run(`(()=>{const t=TemplateLibraryRuntime.get().templates.find(t=>t.name==='중첩목록표준.docx');if(t.registration.version!==15||!t.bindings.some(b=>b.check_id==='ITSEC-12'))throw Error('version 12 migration');return true;})()`);
