...
def build_guide(version, out_path):
    ...
    rows = [
        ...
        ("검토자 확인 완료", "검토자가 이상없음으로 확인한 항목입니다. 펼치면 근거 조항과 판정을 다시 볼 수 있습니다."),
    ]
    for left, right in rows:
        cells = table.add_row().cells
        _add_text(cells[0], left, True)
        _add_text(cells[1], right)
    _table_geometry(table, [2700, 6660])
    _callout(doc, "기억할 점", "기본은 관찰 모드이며 초기 승인 규칙은 없습니다. 내부 독립 검수·시험·승인을 통과한 좁은 규칙만 본건 원문·별첨과 적용범위를 확인한 뒤 제한 적용합니다. 유사도는 이상없음의 근거가 아닙니다. 자료 변경·만료 시 재확인하며 최종 책임은 검토자에게 있습니다.", GOLD)
    ...
    _body(doc, "기존 시스템 이상없음은 완료 건수에서 제외됩니다. 검토자가 원문을 다시 확인한 뒤 이상없음 또는 검토의견을 남기세요. 사람의 직접 판정은 보존됩니다.")
