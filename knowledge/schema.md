...
checks:
  - id: OUT-09-1               # 전역 유일. 유형약어-번호(-원자순번), 조문 항·호 = 1 항목
    ...
    fund_scope: [private]      # 선택. 펀드 성격(public|private)별 적용. 계약서 문언으로 자동 판별(11.5차)
    party_roles: [질권자, 담보권자]       # 선택. 당사가 이 지위일 때만 적용. 지위 미상이면 통과(11.1차)
    subject_roles: [집합투자업자]         # 선택. 이 항목의 주체(수범자). 조항 주어와 대조해 가감(11.6차)
    ...
    llm_elements:                  # 선택: 로컬 LLM 충족도 판정용, 사람이 확정한 필수 요소
      - 재위탁 제한 또는 금지
      - 위탁자의 사전 동의
    service_scope: [completion]    # 선택(12차): 용역 성질결정 게이트 — completion(도급형)|mandate(위임형).
...
### 펀드 성격 분기 (fund_scope, 11.5차)

일반 사모집합투자기구는 자본시장법 **§249-8**이 상당수 조항의 적용을 배제한다:
- ①항: §188②·③(신탁계약 변경 시 수익자총회 결의·변경 공시통지) 배제
- ⑤항: **집합투자자총회 및 그와 관련된 사항 전체** 배제 → §190 총회·§191 매수청구권 성립 불가
- ⑥항: 규약에서 정한 방법으로 전체 투자자에게 통지하면 공시·공고를 갈음

따라서 사모 신탁계약서가 수익자총회·매수청구권을 두지 않는 것이 **정상**이며, 공모 기준 체크를
붙이면 법적으로 틀린 지적이 된다(2026-08-05 실사고). `fund_scope: [public]` / `[private]`로 분기하고,
판별은 **계약서 문언으로 자동**(검토자에게 묻지 않음). 판별 불가 시 게이트 비활성(누락검출 우선).

§79(선관·충실)·§246(수탁 독립성·분별관리)는 배제 목록에 없어 사모에도 적용된다.

### 당사자 축 — 조항 주어 (subject_roles, 11.6차)

한국어 계약 조항은 주체를 문두에 '지위어+은/는'으로 명시하는 것이 표준이다
("집합투자업자는 …하여야 한다" / "수익자는 …할 수 있다"). 같은 문언이라도 **주체가 누구냐**에
따라 검토 의미가 달라지므로, `ClauseRole.clauseSubjects`가 각 항(①②…) 첫머리에서 주어를 뽑아
체크의 `subject_roles`와 대조한다.

| 상태 | 효과 |
|---|---|
| 주어 ∈ subject_roles | `+SUBJECT_BONUS`(6) |
| 주어가 읽혔으나 불일치 | `-SUBJECT_PENALTY`(4) |
| 주어 미상 / `subject_roles` 미선언 | 0 (중립) |

가중을 표제(10)보다 작게 둔 이유: 한 조항에 여러 주체가 섞이는 경우가 흔해(제35조 ①항 수익자,
②항 집합투자업자) 과신하면 정탐을 깎는다. 페널티를 보너스보다 작게 둔 것도 같은 이유
(누락검출 우선). 주어 미상을 감산하지 않는 것은 다른 게이트와 동일한 "모름을 근거로 접지 않음" 원칙.

실측(2026-08-05 사용자 사례): "제35조(반대수익자 매수청구권)" ①항 주어가 '수익자'라
매수청구권 체크는 +6, 운용사 의무 체크(총회 결의·공시)는 −4 → 10점 격차로 오부착이 줄어듦.

세 축 요약 — **문서 제목**(무슨 계약인가) · **조 표제**(이 조항이 무엇인가) · **조항 주어**(누구의
의무·권리인가)가 매칭 정밀도의 기준축이다.

### 조항 귀속 (clause ownership, 11.3차)

조항 표제가 특정 체크를 정면으로 지시하면(표제 적합도 ≥ `TITLE_STRONG_RATIO`) 그 조항은 그 체크의 것.
다른 체크가 같은 조항을 best로 집어가면 "엉뚱한 체크가 붙는" 오탐이 된다.

실사고(2026-08-05): "제35조(반대수익자의 수익증권매수청구권)"라고 표제가 명확한데도 수익자총회
결의·변경 공시 체크가 함께 붙음. 매수청구권 조항이 요건을 적으면서 "신탁계약의 변경에 대한
수익자총회의 결의에 반대하는 경우"라고 **다른 제도를 전제로 참조**했기 때문 — 참조이지 규정이 아님.

접는 조건은 좁게 잡는다(한 조항이 여러 체크를 정당하게 충족하는 경우가 많으므로):

> 표제가 남을 가리키고(owner) **+** 내 표제 근거 0 **+** 본문 겹침 < `OWNED_CLAUSE_MIN_OVERLAP`(4)

실측: 오탐(참조 언급)은 겹침 2~3, 정탐("제6조 보호조치" 조항의 접근제한 체크)은 겹침 5.
명시 인용(citation)·표제 강일치는 예외로 항상 통과.
표제 적합도는 `Sim.keywords` 토큰 비교가 아니라 **부분문자열 포함**으로 계산 — 한국어 법령 표제는
"수익증권매수청구권"처럼 복합어 한 덩어리라 토큰이 절대 일치하지 않기 때문(실측 확인).
...
> 게이트는 허용형(OR)이라 지위가 겹쳐 읽히면 양쪽 관점을 모두 노출한다 — 법무 체크리스트에서는
> "덜 보여주는 것"보다 "더 보여주는 것"이 안전하므로 의도된 동작.

**관점이 뒤집히는 쌍의 예 (11.2차)** — 같은 담보설정계약이라도 자리에 따라 체크군이 갈린다:

| 당사 지위 | 체크군 | 보는 것 |
|---|---|---|
| 질권자·담보권자·채권자 | `FIN-SEC-01~05` | 대항요건·점유이전을 갖추었는가(담보의 실효성) |
| 담보제공자·설정자·물상보증인 | `FIN-SEC-P01~P04` | 유질 약정(민법 §339 위반) 유무, 피담보채무 범위·최고액 특정(§357①), 구상권 보전(§341), 처분제한 범위(§352) |

담보제공자 국면은 실무 빈도가 낮으므로(사용자 확인) 원문 근거가 분명한 4개 항목만 둔다 — 과설계 금지.
