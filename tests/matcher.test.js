...
test("analyze: verify(weak-role) reason은 문구 확인 권장 안내", () => {
  const r = analyze(CLAUSES, [OUT_DOC], ["M-CORE"]);
  const biz = r.results.filter((x) => x.cpId === "CMN-02" || x.cpId === "BIZ");
  // 목적/정의 계열이 verify로 잡히면 안내 문구 확인
  const verifyWeak = r.results.filter((x) => x.coverage === "verify" && x.best &&
    x.best.reasons.some((s) => s.indexOf("확인 권장") !== -1));
  assert.ok(verifyWeak.length >= 1);
  void biz;
});
