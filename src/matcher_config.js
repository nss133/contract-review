...
var MatcherConfig = {
  ...
  TITLE_BONUS_MAX: 5,   // 표제 용어 겹침 가산 상한(어휘 개수 기반 — 복합어 표제엔 거의 무력)
  // 조 표제 직접 대응 가산 상한(11.5차) — titleFitRatio(부분문자열 기반)에 비례.
  // "제35조(반대수익자 매수청구권)"이 그 체크를 정면 지시할 때 결정적 신호가 되도록 크게 둠.
  // tfidf 성분(최대 ~80)과 겨룰 수 있는 크기라야 표제가 실제로 매칭을 좌우함.
  CLAUSE_TITLE_BONUS_MAX: 10,
  // 당사자 축(11.6차) — 조항 주어가 체크의 subject_roles와 일치/불일치할 때 가감.
  // 표제 가중(10)보다 작게 둠: 주어는 보조 신호이고, 한 조항에 여러 주체가 섞이는
  // 경우가 흔해 과신하면 정탐을 깎음. 페널티는 보너스보다 작게(누락검출 우선).
  SUBJECT_BONUS: 6,
  SUBJECT_PENALTY: 4,
  ...
  // 구조화 태그 매칭. shadow는 추적정보만 기록하고 기존 점수·판정을 전혀 바꾸지 않는다.
  // assist로 승격해도 curated signature만 사용하고, 태그 자체는 노출/자동판정 증거가 아니다.
  TAG_MATCH_MODE: "shadow", // Node·직접 로드 안전 기본값. 배포 HTML은 검수 16개에 assist를 주입.
  TAG_BONUS_CAP: 18,
  TAG_PENALTY_CAP: 12,
  ...
  DETECT_FILENAME_W: 6,   // 업로드 파일명 키워드 가중(2026-08-19 피드백) — 제목(8)보다 약하고
                          //   표제부(3)보다 강하게. 파일명 단독으로도 유형 확정 가능(6×1.0 > 3)
...
// Node 골드셋·비교실험에서 소스 수정 없이 모드를 전환한다. 브라우저 빌드는
// cr-data의 tag_match_mode를 app.js가 적용하므로 process에 의존하지 않는다.
if (typeof process !== "undefined" && process.env &&
    ["off", "shadow", "assist"].indexOf(process.env.CONTRACT_TAG_MATCH_MODE) !== -1) {
  MatcherConfig.TAG_MATCH_MODE = process.env.CONTRACT_TAG_MATCH_MODE;
}
