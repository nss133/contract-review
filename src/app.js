...
function renderReport() {
  ...
  // N/M 분모 병기(피드백 5차): "숫자만으론 3개 중 몇 개인지 안 읽힘 — 3/3이어야 다 체크됐구나 함".
  //   확인 안 된 항목 = 판정 완료 M / 전체 알람 N(분모는 판정해도 고정 — 0/3→3/3 진행이 읽히게).
  //   반영 = 반영 N / 이번 검토에서 대조한 활성 검토항목 M. 형식 = warn N / 점검 항목 M.
  ...
  var activeN = 0; // 이번 분석에서 대조한 활성 검토항목 수(비활성 모듈 제외)
  r.results.forEach(function (res) { if (_cpById(res.cpId)) activeN++; });
  var formalN = (state.formal || []).length;
  right += '<div class="report-tiles">' +
    _tile("rpt-sec-addressed", "tile-addressed", "✓ 계약서에 반영된 항목", addressed.length + " / " + activeN,
      "대조한 검토항목 " + activeN + "건 중 조항에서 확인됨") +
    ...
    _tile("rpt-sec-suggest", "tile-verify", "△ 함께 살펴볼 항목", verifyMain.length,
      "관련 조항이 있어 보임 — 참고 제안") +
    _tile("rpt-sec-formal", "tile-formal", "형식 확인 필요", formalWarns.length + " / " + formalN,
      "형식 점검 " + formalN + "건 중 확인 필요 — 상호·빈칸·날짜 등") +
